...
function pourLiquid(type, jar) {
  ...
  stream.style.background = type === "milk" ? "#fff" : "#4ade80";
  ...
  // Position stream to fall into the center of the cylinder
  // Hydrogen peroxide (milk) - positioned from top-left
  if (type === "milk") {
    stream.style.left = "38.5%";
    stream.style.top = "200px";
  } else {
    // Spinach paste (water) - positioned from top-right  
    stream.style.left = "38.5%";
...
function createBubbles() {
  ...
  bubbleContainer.style.bottom = '175px'; // Position above the liquid level
  bubbleContainer.style.left = '41.5%';
...
function increaseVolume() {
  // Gradually increase the height of both liquid fills to simulate volume increase
  gsap.to(milkFill, {
    height: "+=20",
    duration: 3,
    ease: "power2.out"
  ...
  setTimeout(() => {
    ...
    volumeIndicator.innerHTML = '<strong>Volume increased!</strong><br>Initial: 20ml → Final: ~25ml';
    volumeIndicator.style.position = 'absolute';
    volumeIndicator.style.top = '50px';
    volumeIndicator.style.right = '20px';
    volumeIndicator.style.background = 'rgba(16, 185, 129, 0.2)';
    volumeIndicator.style.color = '#10b981';
    volumeIndicator.style.padding = '10px';
    volumeIndicator.style.borderRadius = '8px';
    volumeIndicator.style.border = '1px solid #10b981';
