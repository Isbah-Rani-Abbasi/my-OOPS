...
function pourLiquid(type, jar) {
  ...
  if (type === "milk") {
    ...
    stream.style.left = "43.5%";  // Adjusted to center of cylinder
  ...
  gsap.to(fill, {
    ...
    onComplete: () => {
      ...
      if (milkAdded && waterAdded) {
        ...
        milkFill.style.background = "#166534";  // Dark green
        milkFill.style.opacity = "0.8";
        waterFill.style.background = "#166534"; // Dark green
        waterFill.style.opacity = "0.8";
      } else if (type === "water") {
...
function createBubbles() {
  ...
  bubbleContainer.style.left = '43%';    // Centered with cylinder
  bubbleContainer.style.width = '50px';   // Width to fit inside cylinder
...
function createSingleBubble(container) {
  ...
  // Random horizontal position within container - kept inside cylinder
  bubble.style.left = `${Math.random() * 30 + 10}px`; // 10px to 40px range to stay inside
  bubble.style.bottom = '0px';
  
  container.appendChild(bubble);
  
  // Animate bubble rising - reduced horizontal movement to keep inside cylinder
  gsap.to(bubble, {
    y: -100 - Math.random() * 50,
    x: (Math.random() - 0.5) * 10, // Reduced from 20 to 10 to keep inside
    opacity: 0,
